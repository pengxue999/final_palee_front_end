...
class _PrintDialogState extends State<_PrintDialog>
  ...
  bool get _useManagedPrintDialog =>
  ...
  Future<void> _doPrint() async {
    if (!_useManagedPrintDialog && _selectedPrinter == null) {
      await _doSavePdf();
      return;
    }
  ...
  Widget _buildPreviewSidePanel() {
    if (kIsWeb || _useSystemPrintDialog) {
      final isWeb = kIsWeb;
      return Container(
        width: 290,
        padding: const EdgeInsets.fromLTRB(18, 18, 18, 18),
        ...
        child: Column(
          ...
          children: [
            _buildInfoCard(
              ...
              description: isWeb
                  ? 'ໃນ Web ທ່ານຈະເຫັນ PDF preview ຢູ່ດ້ານຂວາ ແລະ ເມື່ອກົດ ພິມ ລະບົບຈະເປີດໜ້າຕ່າງພິມຂອງ browser.'
                  : 'ໃນ Windows ລະບົບຈະເປີດ system print dialog ເພື່ອຫຼີກບັນຫາ crash ຈາກ direct print ແລະ ໃຫ້ເລືອກ printer ໄດ້ປອດໄພກວ່າ.',
              caption: isWeb
                  ? 'ຖ້າ preview ມາແລ້ວ ສາມາດກົດ ພິມ ຫຼື ບັນທຶກ PDF ໄດ້ທັນທີ.'
...
Future<Uint8List> _buildPdf({
  ...
  try {
    ...
    if (serverError != null) {
      throw Exception(
        'ບໍ່ສາມາດສ້າງໃບລົງທະບຽນ PDF ໄດ້. Server: $serverError | Fallback: $fallbackError',
      );
    }
    throw Exception('ບໍ່ສາມາດສ້າງໃບລົງທະບຽນ PDF ໄດ້: $fallbackError');
  }
}
